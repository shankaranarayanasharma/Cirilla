...
class AppServiceInject implements AppService {
  ...
  Dio _createDio() =>
      _singletonDio ??= _networkModule.provideDio(_createPersistHelper());

  DioClient _createDioClient() =>
      _singletonDioClient ??= _networkModule.provideDioClient(_createDio());

  RequestHelper _createRequest() => _singletonRequestHelper ??=
      _networkModule.provideRequestHelper(_createDioClient());
