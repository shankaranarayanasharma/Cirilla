...
import 'package:cirilla/screens/cart/cart_custom_screen.dart';
...
class _ProductScreenState extends State<ProductScreen>
  ...
  Future<void> _handleAddToCart([bool? goCartPage]) async {
    ...
    try {
      ...
      if (_product!.type == ProductType.variable) {
        ...
        print("Current Product Qty : "+_product!.id.toString());
        // Navigator.push(
        //   context,
        //   MaterialPageRoute(builder: (context) => const CartCustomScreen()),
        // );
      ...
      Widget? bottomBar = enableBottomBar
          ? ProductBottomBar(
              ...
              product: _variationStore != null && _variationStore!.productVariation != null
                  ...
                  : _product,
              // onPress: (int){
              //   showSuccess(context, _product!.id.toString());
              //   Navigator.push(
              //     context,
              //     MaterialPageRoute(builder: (context) => const CartCustomScreen()),
              //   );
                ...
                children: [
                  Flexible(
                    ...
                    child: Column(
                      ...
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ProductRating(product: _product, align: align),
                        Container(
                            margin: const EdgeInsets.only(top: 5),
                            child: ProductCategoryList(product: _product, align: align)
                        ),
                        ProductName(product: _product, align: align)
                      ],
                    ),
                  ),
                  Flexible(
  ...
  Widget buildSlideshow(WidgetConfig configs) {
    ...
    // Config size
    dynamic size = get(configs.fields, ['productGallerySize'], {'width': 375, 'height': 300});
    double? width = ConvertData.stringToDouble(size is Map ? size['width'] : '375');
    double? height = ConvertData.stringToDouble(size is Map ? '350' : '300');
